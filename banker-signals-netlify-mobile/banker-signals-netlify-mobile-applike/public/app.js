...
$('#testPushBtn').addEventListener('click', async () => {
  try {
    await showLocalNotification('✅ Cleanest Top Banker Legs', 'This is a test banker notification from your app.');
...
$('#testPushBtnTwo').addEventListener('click', async () => {
  try {
    if (!pushConfigured) {
      showInfo('Server push is not connected on this Netlify app yet. Add the VAPID keys in Netlify first, then this button will send a real push notification.');
      return;
    }
    await ensurePushSubscription();
...
// Keep the web app feeling like an app on mobile.
